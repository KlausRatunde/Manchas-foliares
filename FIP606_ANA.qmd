...
```{r}
medias_prod <- emmeans(m1, ~ trat)
medias_prod
```

### Atribuindo diferença ou não às médias
...
Como visto, não há diferença significativa entre as médias das produções.

Teste termina acima.

Abaixo temos apenas algumas outras etapas que podem ser realizadas.
...
# sev$sev_parcela <- as.numeric(sev$sev_parcela)
```
...
medias_ymax <- emmeans(ymax, ~ bloco)
medias_ymax
...
sev3 |>
  group_by(trat, sev_parcela) |> 
  summarise(mean_sev = mean(sev_parcela)) |> 
  ggplot(aes(trat, mean_sev)) +
  geom_boxplot() +
  geom_jitter(width = 0.2, color = "gray")+
  stat_summary(fun.data = "mean_cl_boot", color = "black", size = 0.5)+
  theme_classic() +
...
```{r}
medias_pl_r <- emmeans(m_curva_pl_r, ~ trat)
medias_pl_r
```

```{r}
cld(medias_pl_r, Letters = letters)
```
